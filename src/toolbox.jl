using LinearAlgebra, Random, SparseArrays,SIMD, StatsBase
...
function idx2pair(idx::Int64,n::Int64)
    i = (idx-1)÷(n-1)+1;
    j = (idx-1)%(n-1)+1;
    j += (j>=i);
    return (i,j)
...
function pair2idx(i::Int64,j::Int64,n::Int64)
    if j > i
        j = j - 1
    end
    idx = (i-1)*(n-1)+(j-1)+1
...
function compute_randomized_infeasibility(Xflat::Array{T,1},n::Int64,d::Int64,phi::Array{T,1},xi::Array{T,1}, div::Int) where T<:AbstractFloat
    infeas = 0.
    max_viol = 0.
    constr = 0.
    m = n ÷ div;
    samples = sample(1:n, m, replace=false)
    for i in samples
        @simd for j in 1:n
            if j != i
                @inbounds constr = phi[j]-phi[i]+ dot(Xflat[index2d(i,d)]-Xflat[index2d(j,d)], xi[index2d(i,d)])
...
function get_primal_feasible_solution(Xflat::Array{T,1},Y::Array{T,1},rho::T,I::Array{Int64,1},J::Array{Int64,1}, Wlen::Int64, lamb::Array{T,1},phi::Array{T,1},xi::Array{T,1}, n::Int64, d::Int64) where T<:AbstractFloat
    ...
    for j in 1:n
        ...
        for i in 1:n
            ...
            overall_max_viol = min(max_viol,overall_max_viol)
        end
    ...
    mean_max_viol /= n;
    phi_f .+= mean_max_viol;
    return phi_f, xi_f, overall_max_viol, infeas
end
...
function get_dual_solution_quality(Xflat::Array{T,1},Y::Array{T,1},rho::T,I::Array{Int64,1},J::Array{Int64,1}, Wlen::Int64, lamb::Array{T,1},phi::Array{T,1},xi::Array{T,1}, n::Int64, d::Int64; return_feasible_soln=false,return_gap=true,randomized=false,div=10) where T<:AbstractFloat
    s_tmp = zeros(Wlen)
    L_upper = get_objective_value(Xflat,Y,rho,I,J, Wlen, lamb,n, d, phi, xi, s_tmp; get_phi_xi=true);
    if return_gap
        phi_f,xi_f, max_viol, infeas = get_primal_feasible_solution(Xflat, Y, rho, I, J, Wlen, lamb, phi, xi, n, d);
        L_lower = -(0.5*norm(phi_f-Y)^2+rho/2*norm(xi_f)^2)
        if return_feasible_soln
            return L_upper-L_lower, infeas, max_viol, phi_f, xi_f, L_upper, L_lower
        else
            return L_upper-L_lower, infeas, max_viol, L_upper, L_lower
        end
    else
        if ~randomized
            infeas, max_viol = compute_infeasibility(Xflat,n,d,phi,xi);
            return NaN, infeas, max_viol, L_upper, NaN
        else
            infeas, max_viol = compute_randomized_infeasibility(Xflat,n,d,phi,xi,div);
            return NaN, infeas, max_viol, L_upper, NaN
        end
    end
...
# function predict!(y_pred,Xtest,phi_hat,xi_hat,ntrain,d,phi_minus_xi_dot_x)
#     fill!(y_pred,0)
#     ntest =size(Xtest,1)
#     Xi_hat = reshape(copy(xi_hat'),(n,d))
#     y_pred.= map(z->maximum(z), eachslice(Xi*Xtest'.+phi_minus_xi_dot_x,dims=2))
# end


# function predict(Xtest,phi_hat,xi_hat,ntrain,d,phi_minus_xi_dot_x)
#     ntest =size(Xtest,1)
#     y_pred = zeros(ntest)
#     predict!(y_pred,Xtest,phi_hat,xi_hat,ntrain,d,phi_minus_xi_dot_x)
#     return y_pred
# end
